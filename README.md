...
**<http://localhost:7073/studio>** — it shows whether things are working and the exact steps to
connect Kineviz.
...
| ✎ **You decide** | Both directions join. No query can settle it — read the row and pick the sentence that is true. |
...
it is a list keyed by name, so `Associate` cannot appear three times. We report the **most
...
**Not supported** — and each one is **refused with a message**, never quietly mistranslated:
...
and SQL Server refuses more than 2,100 parameters in a single statement. The bridge sends large
lists as one JSON parameter instead, so this works; it is only worth knowing because the
obvious implementation cannot.
