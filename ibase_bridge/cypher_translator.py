...
The backend compiles it to T-SQL.
"""
...
def _parse_edge(conn: str, idx: int, left: str, right: str) -> Optional[EdgePat]:
    ...
    if bm:
        ...
        # Variable-length paths ([:T*], [:T*1..3]) are not translated yet. T-SQL
        # *could* express them with a recursive CTE, so this is a "not built" rather
        # than a "cannot be done" - unlike the PostgreSQL bridge, where the engine
        # itself has no path quantifier. Reject clearly instead of emitting a broken label.
        if re.search(r"\*|\+|\.\.|\{", body):
            raise TranslateError(
                "variable-length paths (for example [:KNOWS*1..3]) are not "
                "supported yet. Write the hops out explicitly, or expand one step "
                "at a time from the canvas.")
