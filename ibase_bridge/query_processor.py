...
_REFUSALS_CACHE = None


def _refusals():
    global _REFUSALS_CACHE
    if _REFUSALS_CACHE is None:
        _REFUSALS_CACHE = _deliberate_refusals()
    return _REFUSALS_CACHE


def _deliberate_refusals():
    """Exceptions that mean "we will not run this", as opposed to "this branch found
    nothing". These must reach the user; everything else can be skipped."""
    out = []
    try:
        from .tsql import UnsupportedByTSql
        out.append(UnsupportedByTSql)
    except ImportError:
        pass
    try:
        from .mssql_backend import BranchLimitExceeded
        out.append(BranchLimitExceeded)
    except ImportError:
        pass
    return tuple(out) or (ZeroDivisionError,)   # never an empty tuple
...
class QueryProcessor:
    ...
    def execute(self, query: str) -> QueryOutcome:
        ...
        if "call test" in ql:
            ...
            return QueryOutcome("TABLE", [["status"], ["test() is a no-op in this bridge"]], self.summary())
        ...
        # relationship type (each generated query needs a concrete table). Run each and
        ...
        for bp in branches:
            ...
            try:
                ...
                # The bridge decided it will not run this - an unsupported operator,
                # or a fan-out too large to be sensible. That is a decision, not a
                # failed branch, and the user must see it. Skipping it here would
                # hand back an empty graph, which reads as "nothing matched".
                raise
            except Exception as e:
                # A branch that is simply unsatisfiable - say, an endpoint label that
                # cannot occur for this link type - is normal. Skip it and carry on.
