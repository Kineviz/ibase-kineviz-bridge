...
class SqlCtx:
    """Everything `to_sql` needs, including how this database spells things.

    The parser and the expression tree above are engine-neutral and shared with
    the PostgreSQL bridge unchanged. Only the SQL *spelling* differs, and it all
    arrives through `dialect` (see `tsql.Dialect`), so there is one seam rather
    than a forked file.
    """

    def __init__(self, gen, label_of, node_pk, registry, params,
                 dialect=None, param_types=None, column_types=None,
                 node_columns=None):
        self.gen = gen                 # var -> generated pattern var (v0)
        self.label_of = label_of       # var -> label
        self.node_pk = node_pk         # label -> pk column
        self.registry = registry       # NodeIdCodec: reverses internal_id(t, o)
        self.params = params           # list, appended to
        self.dialect = dialect
        # Parallel to `params`. pyodbc binds every Python str as nvarchar, and an
        # nvarchar parameter against a varchar column makes SQL Server convert the
        # COLUMN — an index lookup silently becomes a full scan. Carrying each
        # parameter's real column type lets the connection layer bind it correctly.
        self.param_types = param_types if param_types is not None else []
        self.column_types = column_types or {}       # "Label.prop" -> sql type
        self.node_columns = node_columns or {}       # "Label.prop" -> physical column

    def param(self, value, sql_type=None) -> str:
        self.params.append(value)
        self.param_types.append(sql_type)
        return self.dialect.placeholder if self.dialect else "%s"

    def col(self, prop: Prop) -> str:
        gen = self.gen.get(prop.var, prop.var)
        label = self.label_of.get(prop.var)
        name = self.node_columns.get("{}.{}".format(label, prop.prop), prop.prop)
        if self.dialect:
            return "{}.{}".format(gen, self.dialect.quote(name))
        return "{}.{}".format(gen, name)

    def type_of(self, prop: Prop):
        return self.column_types.get(
            "{}.{}".format(self.label_of.get(prop.var), prop.prop))

    def pk_column(self, label: str) -> str:
        pk = self.node_pk.get(label, "id")
        return self.node_columns.get("{}.{}".format(label, pk), pk)
...
def to_sql(expr, ctx: SqlCtx) -> str:
    ...
    if isinstance(expr, StrOp):
        ...
        sql_type = ctx.type_of(expr.lhs)
        if expr.op == "regex":
            if ctx.dialect:
                # T-SQL has no regex operator. Say so plainly rather than returning
                # an empty result the user might mistake for "no matches".
                return ctx.dialect.regex(col, expr.value)
            return "{}::text ~ {}".format(col, ctx.param(expr.value))
        if ctx.dialect:
            frag, pattern = ctx.dialect.like(col, expr.value, expr.op, sql_type)
            ctx.param(pattern, sql_type)
            return frag
    ...
    if isinstance(expr, InList):
        if not expr.values:
            if ctx.dialect:
                return ctx.dialect.empty_in(expr.negated)
            return "false" if not expr.negated else "true"
        s = _in_list_sql(ctx, ctx.col(expr.lhs), expr.values, ctx.type_of(expr.lhs))
    ...
    if isinstance(expr, IdIn):
        ...
        pk = ctx.pk_column(label)
        gen = ctx.gen.get(expr.var, expr.var)
        col = "{}.{}".format(gen, ctx.dialect.quote(pk)) if ctx.dialect else "{}.{}".format(gen, pk)
        s = _in_list_sql(ctx, col, keys, ctx.column_types.get("{}.{}".format(label, ctx.node_pk.get(label, "id"))))
        return "NOT ({})".format(s) if expr.negated else s
    raise ValueError("cannot compile predicate node: %r" % (expr,))


def _in_list_sql(ctx: "SqlCtx", column: str, values, sql_type=None) -> str:
    """`column IN (...)` — the one place the 2100-parameter ceiling matters.

    Kineviz writes every selected node's id into an Expand query; in real captured
    traffic that is routinely thousands and once was 42,214. SQL Server refuses more
    than 2100 parameters in one statement, so above a threshold the whole list goes
    across as a single JSON parameter that the server unpacks. See `tsql.id_list_sql`.
    """
    if ctx.dialect is None:
        return "{} IN ({})".format(column, ", ".join(ctx.param(v) for v in values))
    from . import tsql
    return tsql.id_list_sql(column, list(values), ctx.params, ctx.param_types,
                            sql_type=sql_type, openjson=ctx.dialect.supports_openjson)
